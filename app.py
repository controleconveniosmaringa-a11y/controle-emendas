    ...
    with tabs_cred[0]:
        ...
        if not df_gestao.empty:
            ...
            if modo_edicao:
                ...
                with st.form("form_edicao_finisa"):
                    df_editado = st.data_editor(df_gestao, use_container_width=True, num_rows="dynamic", key="editor_gestao")
                    st.markdown("<br>", unsafe_allow_html=True)
                    btn_salvar_finisa = st.form_submit_button("💾 Salvar Alterações e Recalcular Automaticamente (GitHub)", type="primary")
                    
                    if btn_salvar_finisa:
                            ...
                            with st.spinner("Calculando matemáticas (Saldos, Porcentagens e Totais) e salvando no GitHub..."):
                                try:
                                    ...
                                    if r_get.status_code == 200:
                                        ...
                                        data_payload = {
                                            "message": "Painel Streamlit: Edição FINISA e Auto-Recálculo de Saldos",
                                            "content": content_b64,
                                            "sha": sha
                                        }
        ...
        with tab_conv_geral:
            ...
            if busca_global:
                ...
                if modo_edicao_conv:
                    ...
                    with st.form("form_edicao_convenios"):
                        df_editado_conv = st.data_editor(df_conv_tela, use_container_width=True, num_rows="dynamic", key="editor_conv")
                        st.markdown("<br>", unsafe_allow_html=True)
                        btn_salvar_conv = st.form_submit_button("💾 Salvar Convênios Atualizados (GitHub)", type="primary")
