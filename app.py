...
st.markdown("""<style>
    ...
    :root { --bg-main: #f8fafc; --text-main: #0f172a; --text-muted: #64748b; --card-bg: #ffffff; --card-border: #e2e8f0; --card-hover: #f1f5f9; --header-bg: #0f172a; --header-text: #f8fafc; --table-bg: #ffffff; --table-border: #cbd5e1; --table-th-bg: #1e293b; --table-th-text: #f8fafc; --table-row-even: #f8fafc; --table-row-hover: #f1f5f9; --table-final-bg: #ecfdf5; --table-final-text: #065f46; --kpi-blue-bg: #f8fafc; --kpi-blue-border: #bfdbfe; --tag-bg: #f1f5f9; --tag-border: #cbd5e1; --tag-text: #334155; --link-bg: #eff6ff; --link-text: #2563eb; --link-hover-bg: #dbeafe; --search-bg: #f0f9ff; --search-border: #bae6fd; --success-val: #059669; --danger-val: #dc2626; --blue-val: #2563eb; --purple-val: #6366f1; --warning-val: #d97706; --warning-bg: #fffbeb; }
    @media (prefers-color-scheme: dark) { :root { --bg-main: #0e1117; --text-main: #f8fafc; --text-muted: #94a3b8; --card-bg: #1e293b; --card-border: #334155; --card-hover: #0f172a; --header-bg: #020617; --header-text: #f8fafc; --table-bg: #1e293b; --table-border: #334155; --table-th-bg: #0f172a; --table-th-text: #f8fafc; --table-row-even: #0f172a; --table-row-hover: #334155; --table-final-bg: #064e3b; --table-final-text: #34d399; --kpi-blue-bg: #0f172a; --kpi-blue-border: #1e3a8a; --tag-bg: #334155; --tag-border: #475569; --tag-text: #cbd5e1; --link-bg: #1e3a8a; --link-text: #93c5fd; --link-hover-bg: #1e40af; --search-bg: #0c4a6e; --search-border: #0284c7; --success-val: #34d399; --danger-val: #f87171; --blue-val: #60a5fa; --purple-val: #a78bfa; --warning-val: #fbbf24; --warning-bg: #451a03; } }
...
def obter_base_credito():
    ...
    try:
        df_raw = pd.read_csv(url, low_memory=False, dtype=str, keep_default_na=False, na_filter=False, on_bad_lines='skip')
    except Exception: return pd.DataFrame(), "Aguardando envio pelo Google Sheets"
    if df_raw.empty: return pd.DataFrame(), "Base Vazia"
    col_orig = {re.sub(r'[^\w\s]', '', str(c).strip().lower()).replace('â', 'a').replace('ç', 'c').replace('ã', 'a').replace('ó', 'o'): c for c in df_raw.columns}
    def ext_c(chave_exata, chave_parcial):
        for limpo, orig in list(col_orig.items()):
            if chave_exata in limpo: del col_orig[limpo]; return [str(i).strip() if str(i).strip().lower() not in ['', 'nan'] else '' for i in df_raw[orig]]
        for limpo, orig in list(col_orig.items()):
            if chave_parcial in limpo: del col_orig[limpo]; return [str(i).strip() if str(i).strip().lower() not in ['', 'nan'] else '' for i in df_raw[orig]]
        return [''] * len(df_raw)
    df = pd.DataFrame()
...
def obter_base_gestao_convenios():
    ...
    try:
        ...
        if r.status_code == 200:
            file_info = r.json()
            content_bytes = base64.b64decode(file_info['content'])
            try: df = pd.read_csv(io.BytesIO(content_bytes), sep=';', low_memory=False, dtype=str, keep_default_na=False, na_filter=False, on_bad_lines='skip', encoding='utf-8')
            except Exception: df = pd.read_csv(io.BytesIO(content_bytes), sep=';', low_memory=False, dtype=str, keep_default_na=False, na_filter=False, on_bad_lines='skip', encoding='latin1')
            if len(df.columns) < 3:
                try: df = pd.read_csv(io.BytesIO(content_bytes), sep=',', low_memory=False, dtype=str, keep_default_na=False, na_filter=False, on_bad_lines='skip', encoding='utf-8')
                except Exception: df = pd.read_csv(io.BytesIO(content_bytes), sep=',', low_memory=False, dtype=str, keep_default_na=False, na_filter=False, on_bad_lines='skip', encoding='latin1')
    ...
    cache_buster = int(time.time())
    url_raw = f"https://raw.githubusercontent.com/{repo}/main/Gest%C3%A3o%20de%20Conv%C3%AAnios.csv?v={cache_buster}"
    try:
        df = pd.read_csv(url_raw, sep=';', low_memory=False, dtype=str, keep_default_na=False, na_filter=False, on_bad_lines='skip', encoding='utf-8')
        return processar_df_gestao(df)
...
def obter_base_maringa_csv():
    ...
    try:
        ...
        if not df.empty:
            ...
            if col_data: df['Data_Parse'] = pd.to_datetime(df[col_data], dayfirst=True, errors='coerce').fillna(pd.Timestamp('1900-01-01'))
            else: df['Data_Parse'] = pd.Timestamp('1900-01-01')
            col_valor = next((c for c in df.columns if 'VALOR' in c.upper()), None)
            if col_valor: df['Valor_Num'] = df[col_valor].apply(limpar_moeda_blindada)
            else: df['Valor_Num'] = 0.0
        return df, att
    except Exception: return pd.DataFrame(), "Falha na consulta"
    ...
    def processar_extrato(url, banco_nome):
        try:
            ...
            def converter_valor_extrato(val):
                ...
                try: num = float(v); return -num if is_deb else num
                except: return 0.0
        ...
        for aba_nome in abas:
            ...
            if nome_programa == "USINA" and ("1" in str(aba_nome)):
                ...
                if abs(despesas_atuais - valor_correto) > 0.01:
                    ...
                    nova_linha = pd.DataFrame([{'DATA': '-', 'EMPENHO': 'AJUSTE', 'FORNECEDOR': 'AJUSTE DE CÁLCULO SISTEMA', 'TIPO DE DOCUMENTO': '-', 'Nº DOCUMENTO': '-', 'DESCRIÇÃO': 'VALOR FIXADO PELO SISTEMA (AJUSTE DE LANÇAMENTO)', 'REF VALOR REPASSADO': aba_nome, 'LINK DOCUMENTO': '-', 'REPASSE': 0.0, 'RENDIMENTO': 0.0, 'VALOR DESPESA': diferenca}])
            ...
            dados_finais[aba_nome] = {'repasse_atual': repasse_puro_atual, 'rendimento_atual': rendimento_atual, 'saldo_anterior': saldo_anterior, 'total_disponivel': recurso_disponivel_total, 'total_despesa': despesas_atuais, 'saldo_final': saldo_remanescente_final, 'df_filtrado': df_aba}
...
if st.session_state.pagina_atual == 'menu_principal':
    ...
    with c_res:
        if busca_conv_home:
            if not df_conv.empty:
                ...
                for col in df_conv.columns: mask_home |= df_conv[col].astype(str).str.contains(busca_conv_home, case=False, na=False)
                df_encontrado_home = df_conv[mask_home]
                if not df_encontrado_home.empty and 'RESPONSÁVEL' in df_encontrado_home.columns:
                    analistas_unicos = [a for a in df_encontrado_home['RESPONSÁVEL'].unique() if str(a).strip() != ""]
                    if analistas_unicos:
                        for analista in analistas_unicos: st.markdown(f'''<div style='background: var(--link-bg); border: 1px solid var(--blue-val); border-left: 4px solid var(--blue-val); border-radius: 6px; padding: 12px 16px; margin-bottom: 8px;'><div style='font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: 700; margin-bottom: 4px;'>Analista Encontrado</div><div style='font-size: 15px; font-weight: 800; color: var(--text-main); display: flex; align-items: center; gap: 8px;'>👤 {analista}</div></div>''', unsafe_allow_html=True)
                    else: st.warning("⚠️ Registro localizado, mas sem analista associado.")
                else: st.error("❌ Nenhum registro encontrado.")
        else: st.markdown("<div style='border: 1px dashed var(--card-border); border-radius: 6px; padding: 20px; text-align: center; color: var(--text-muted); font-size: 13px;'>Os resultados da sua busca aparecerão aqui.</div>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    if not df_tesouro.empty:
        ...
        st.markdown("<p style='font-size: 13px; color: var(--text-muted); margin-top: -10px; margin-bottom: 15px;'>⚠️ <b>Por que não atualiza diariamente?</b> O painel lê a base de dados em tempo real. Se a data acima não é de hoje, a automação do Governo Federal ainda não publicou os dados de hoje.</p>", unsafe_allow_html=True)
        ...
        for _, r in df_tesouro_sorted.iterrows():
            ...
            st.markdown(f'''<div style='padding: 10px 0; border-bottom: 1px dashed var(--card-border);'><div style='display: flex; justify-content: space-between; align-items: flex-start; gap: 10px;'><div style='flex: 1; min-width: 0;'><div style='font-size: 11px; color: var(--text-muted); font-weight: 700; margin-bottom: 4px;'>📅 {data_t} &nbsp;|&nbsp; Mês: {mes_t} &nbsp;|&nbsp; Emenda: {emenda} &nbsp;|&nbsp; Categoria: {categoria}</div><div style='font-size: 12px; color: var(--text-main); line-height: 1.4; word-wrap: break-word;'><b>Favorecido:</b> {favorecido}</div></div><div style='font-size: 14px; font-weight: 800; color: var(--purple-val); white-space: nowrap; padding-top: 2px;'>{valor_fmt}</div></div></div>''', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

elif st.session_state.pagina_atual == 'resumo_emendas':
    st.button("⬅️ Voltar ao Menu Principal", key="btn_voltar_resumo", on_click=mudar_pagina, args=('menu_principal',))
    ...
    if not df.empty:
        ...
        with c_ag:
            st.markdown(f'''<div class='kpi-card-head' style='border-left: 5px solid var(--warning-val); margin-bottom: 15px;'><div class='kpi-label'>⏳ Aguardando Recursos</div><div class='kpi-value' style='color:var(--warning-val);'>{len(df_aguardando)} Emenda(s)</div></div>''', unsafe_allow_html=True)
    ...
    st.button("⬅️ Voltar ao Menu Principal", key="btn_voltar_credito", on_click=mudar_pagina, args=('menu_principal',))
    ...
    c1, c2 = st.columns(2, gap="large")
    with c1:
        st.markdown(f"<div style='background:var(--card-bg); padding:20px; border-radius:10px; border:1px solid var(--card-border);'><h3 style='color:var(--blue-val); margin:0;'>🏛️ Programa FINISA</h3></div>", unsafe_allow_html=True)
        st.button("Acessar FINISA", key="btn_finisa_card", use_container_width=True, type="primary", on_click=mudar_pagina, args=('finisa',))
    with c2:
        st.markdown(f"<div style='background:var(--card-bg); padding:20px; border-radius:10px; border:1px solid var(--card-border);'><h3 style='color:var(--success-val); margin:0;'>☀️ Usina Fotovoltaica</h3></div>", unsafe_allow_html=True)
        st.button("Acessar Usina", key="btn_usina_card", use_container_width=True, type="primary", on_click=mudar_pagina, args=('fotovoltaica',))

elif st.session_state.pagina_atual == 'finisa':
    st.button("⬅️ Voltar para Operações de Crédito", key="btn_voltar_finisa", on_click=mudar_pagina, args=('credito',))
    ...
    with tabs_cred[0]:
        ...
        if not df_gestao.empty:
            ...
            if col_dot and col_pct and col_pago and col_saldo:
                ...
                if not df_saldo_disp.empty:
                    ...
                    fig_bar_s = go.Figure(go.Bar(x=df_saldo_disp['saldo_num'], y=nomes_completos, orientation='h', marker_color='#10b981', text=[fmt(v) for v in df_saldo_disp['saldo_num']], textposition='auto'))
            ...
            if modo_edicao:
                ...
                if st.button("💾 Salvar Alterações e Recalcular Automaticamente (GitHub)", type="primary"):
                            ...
                            try:
                                def recalcular_tabela_finisa(df_calc):
                                    ...
                                    def formata_pct(v):
                                        ...
                                        return s + "%"
                                    total_aprov = 0.0; total_pago = 0.0; idx_categoria = -1; sub_aprov = 0.0; sub_pago = 0.0
                                    for i, row in df_calc.iterrows():
                                        ...
                                        if "TOTAL" in proj or "TOTAIS" in proj or "TOTAL" in dot.upper() or "TOTAIS" in dot.upper():
                                            ...
                                            if col_pct: df_calc.at[i, col_pct] = ""
                                        elif dot == "" and proj != "":
                                            if idx_categoria != -1:
                                                df_calc.at[idx_categoria, col_aprov] = formata_br(sub_aprov)
                                                df_calc.at[idx_categoria, col_pago] = formata_br(sub_pago)
                                                df_calc.at[idx_categoria, col_saldo] = formata_br(sub_aprov - sub_pago)
                                                if col_pct: df_calc.at[idx_categoria, col_pct] = ""
                                            idx_categoria = i; sub_aprov = 0.0; sub_pago = 0.0
                                        elif dot != "":
                                            val_aprov = limpar_moeda_blindada(row.get(col_aprov, '0'))
                                            val_pago = limpar_moeda_blindada(row.get(col_pago, '0'))
                                            val_saldo = val_aprov - val_pago
                                            pct = (val_pago / val_aprov * 100) if val_aprov > 0 else 0.0
                                            df_calc.at[i, col_aprov] = formata_br(val_aprov)
                                            df_calc.at[i, col_pago] = formata_br(val_pago)
                                            df_calc.at[i, col_saldo] = formata_br(val_saldo)
                                            if col_pct: df_calc.at[i, col_pct] = formata_pct(pct)
                                            sub_aprov += val_aprov; sub_pago += val_pago; total_aprov += val_aprov; total_pago += val_pago
                                    if idx_categoria != -1:
                                        df_calc.at[idx_categoria, col_aprov] = formata_br(sub_aprov)
                                        df_calc.at[idx_categoria, col_pago] = formata_br(sub_pago)
                                        df_calc.at[idx_categoria, col_saldo] = formata_br(sub_aprov - sub_pago)
                                ...
                                if r_get.status_code == 200:
                                    ...
                                    csv_novo = df_editado_calculado.to_csv(index=False, sep=';', encoding='utf-8')
                                    content_b64 = base64.b64encode(csv_novo.encode('utf-8')).decode('utf-8')
                                    data_payload = {"message": "Painel Streamlit: Edição FINISA e Auto-Recálculo de Saldos", "content": content_b64, "sha": sha}
                                    ...
                                    else: st.error(f"Erro ao tentar salvar no GitHub: {r_put.text}")
                                else: st.error("Não foi possível localizar o arquivo atual no GitHub para atualização.")
                            except Exception as e: st.error(f"Erro no sistema: {str(e)}")
            else:
                ...
                for _, row in df_gestao.iterrows():
                    ...
                    if tem_dotacao: bg_cor = "var(--table-bg)"; borda_cor = "1px dashed var(--card-border)"; fonte_peso = "normal"
                    else: bg_cor = "rgba(37, 99, 235, 0.12)"; borda_cor = "1px solid var(--blue-val)"; fonte_peso = "800"
                    td_html = ""
                    for c in valid_cols:
                        val = str(row.get(c, ''))
        ...
        else: st.info("ℹ️ Tabela 'Gestão de Convênios.csv' não encontrada ou está vazia.")
    ...
    for i, aba_nome in enumerate(abas_exibicao):
        with tabs_cred[i+1]:
            ...
            with cg1:
                ...
                st.plotly_chart(fig_rosca, use_container_width=True, key=f"pie1_finisa_{i}")
            with cg2:
                ...
                if not df_desc.empty:
                    ...
                    st.plotly_chart(fig_bar_desc, use_container_width=True, key=f"bar1_finisa_{i}")
    ...
    st.button("⬅️ Voltar para Operações de Crédito", key="btn_voltar_foto", on_click=mudar_pagina, args=('credito',))
        ...
        for i, aba_nome in enumerate(abas_exibicao):
            with tabs_cred[i]:
                ...
                with cg1:
                    ...
                    fig_rosca = go.Figure(data=[go.Pie(labels=['Gasto Liquidado', 'Saldo Disponível'], values=[info['total_despesa'], max(0.0, info['saldo_final'])], hole=.6, marker=dict(colors=['#ef4444', '#10b981']))])
                    fig_rosca.update_layout(height=280, margin=dict(l=10, r=10, t=10, b=10), showlegend=True, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='gray'))
                    st.plotly_chart(fig_rosca, use_container_width=True, key=f"pie1_foto_{i}")
                with cg2:
                    st.markdown("<div class='section-title' style='margin-top:0;'>📊 DESPESAS POR DESCRIÇÃO</div>", unsafe_allow_html=True)
                    df_desc = info['df_filtrado'][info['df_filtrado']['VALOR DESPESA'] > 0].groupby('DESCRIÇÃO')['VALOR DESPESA'].sum().reset_index()
                    if not df_desc.empty:
                        fig_bar_desc = go.Figure(go.Bar(x=df_desc['VALOR DESPESA'], y=df_desc['DESCRIÇÃO'], orientation='h', marker_color='#3b82f6', text=[fmt(v) for v in df_desc['VALOR DESPESA']], textposition='auto'))
                        fig_bar_desc.update_layout(height=280, margin=dict(l=10, r=10, t=10, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color='gray'))
                        st.plotly_chart(fig_bar_desc, use_container_width=True, key=f"bar1_foto_{i}")
    ...
    st.button("⬅️ Voltar ao Menu", key="btn_voltar_conv", on_click=mudar_pagina, args=('menu_principal',))
    ...
    if not df_conv.empty:
        ...
        busca_global = st.text_input("Digite qualquer termo para pesquisar na base inteira:", key="busca_global_conv_aba", placeholder="Ex: Nome de um analista, número SEI, secretaria, fonte...")
        if busca_global:
        ...
        with tab_conv_fonte:
            ...
            if fontes_rec:
                f_final = st.selectbox("Escolha a Fonte:", options=fontes_rec, key="sel_conv_fonte")
                if f_final: st.dataframe(df_conv_tela[df_conv_tela['FONTE DE RECURSO'] == f_final], use_container_width=True)
                            ...
                            with st.spinner("Salvando os convênios na nuvem..."):
                                try:
                                    ...
                                    if r_get.status_code == 200:
                                        ...
                                        content_b64 = base64.b64encode(csv_novo.encode('utf-8')).decode('utf-8')
                                        data_payload = {"message": "Painel Streamlit: Edição Geral de Convênios", "content": content_b64, "sha": sha}
                                        r_put = requests.put(url_api, headers=headers, data=json.dumps(data_payload))
                                        if r_put.status_code in [200, 201]:
    ...
    if not df.empty:
        ...
        st.markdown('''<div class="header-container"><div class="header-left"><div class="main-title">Controle de Emendas Orçamentárias</div></div></div>''', unsafe_allow_html=True)
        aba_selecionada = st.radio("Navegação:", options=["🎯 Por Fonte", "📋 Por Plano", "🏛️ Por Secretaria", "🔍 Por Deputado"], horizontal=True, label_visibility="collapsed", key="radio_nav_emendas")
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        if aba_selecionada == "🎯 Por Fonte":
            ...
            if fontes:
                fonte_final = st.selectbox("Selecione a Fonte:", options=fontes, key="sel_fonte_emendas")
                if fonte_final:
                    df_f = df[df['fonte_clean'] == fonte_final]
                    anos_f = ["Exibir Histórico Acumulado Completo"] + sorted(list(set([str(a) for a in df_f['ano_mov'].unique() if a not in ['', 'nan']])))
                    ano_f_sel = st.selectbox("Exercício Fiscal:", options=anos_f, key="sel_ano_fonte")
                    if not df_f.empty:
                        fluxo_f = df_f if ano_f_sel == anos_f[0] else df_f[df_f['ano_mov'] == ano_f_sel]
                        saldo_f = df_f if ano_f_sel == anos_f[0] else df_f[df_f['ano_mov'].astype(int) <= int(ano_f_sel)]
                        conta_f = df_f['conta corrente'].iloc[0]
                        ...
                        # --- INÍCIO DA BUSCA DO PROCESSO SEI COM FILTRO INTELIGENTE ---
                        ...
                        if not df_conv.empty:
                            ...
                            if col_sei and col_fonte:
                                ...
                                # O FILTRO INTELIGENTE (REGEX)
                                seis_encontrados = []
                                padrao_busca = r'(?i)(sei|execu[cç][aã]o|formaliza[cç][aã]o)'
                                
                                for x in df_sei_match[col_sei].unique():
                                    if pd.notna(x) and str(x).strip() not in ['', '-', 'nan', 'NAN']:
                                        texto_celula = str(x).strip()
                                        if re.search(padrao_busca, texto_celula):
                                            seis_encontrados.append(texto_celula)
                        ...
                        st.markdown(f'''<div style='margin-bottom:10px;'>
                            ...
                            <div class='meta-tag' style='background-color: #fef08a; color: #854d0e; border-color: #fde047;'>📝 {sei_info}</div>
            ...
            if planos:
                p_fin = st.selectbox("Selecione o Plano:", options=planos, key="sel_plano_emendas")
                if p_fin: 
                    ...
                    if not df_plano.empty:
                        ...
                        with c_graf_p:
                            ...
                            st.plotly_chart(fig_rosca_p, use_container_width=True, key="pie_plano")
            ...
            if secs_totais:
                s_fin = st.selectbox("Selecione a Secretaria:", options=secs_totais, key="sel_sec_emendas")
                if s_fin: 
                    ...
                    if not df_sec.empty:
                        ...
                        with c_graf_s:
                            ...
                            st.plotly_chart(fig_rosca_s, use_container_width=True, key="pie_sec")
            ...
            if deps:
                d_fin = st.selectbox("Selecione o Deputado:", options=deps, key="sel_dep_emendas")
                if d_fin: 
                    df_dep = df[df['deputado'] == d_fin]
                    ...
                    if not df_dep.empty:
                        ...
                        st.markdown(f'''<div class='kpi-row-container'><div class='kpi-card-head-blue'><div class='kpi-label'>👤 Parlamentar</div><div class='kpi-value' style='color:var(--blue-val);'>{d_fin}</div></div><div class='kpi-card-head' style='border-left: 5px solid var(--success-val);'><div class='kpi-label'>💰 Saldo Consolidado</div><div class='kpi-value' style='color:var(--success-val);'>{fmt(sal_d)}</div></div><div class='kpi-card-head' style='border-left: 5px solid var(--purple-val);'><div class='kpi-label'>% Disponível</div><div class='kpi-value' style='color:var(--purple-val);'>{pct_disp_d:.2f}%</div></div></div>''', unsafe_allow_html=True)
                        ...
                        with c_graf_d:
                            ...
                            st.plotly_chart(fig_rosca_d, use_container_width=True, key="pie_dep")
