...
# 1. CONFIGURAÇÃO ESTRUTURAL DA PÁGINA (Deve ser o primeiro comando)
st.set_page_config(page_title="Controle de Emendas", page_icon="📊", layout="wide")

# 2. CARREGAMENTO DE DADOS COM CACHE DE LONGA DURAÇÃO (60 MINUTOS)
...
def carregar_base_dados():
    ...
    df['fonte_clean'] = buscar_coluna_original('fonte', '').apply(lambda x: str(x).split('.')[0].strip())
...
# 3. ESTILIZAÇÃO VISUAL CORPORATIVA VIA CSS
st.markdown('''<style>
    ...
    [data-testid="stSidebar"] div[data-testid="stSelectbox"] { background-color: #f1f5f9 !important; padding: 4px; border-radius: 8px; border: 1px solid #cbd5e1; }
    .main-title { font-size: 36px; font-weight: 800; color: #0f172a; letter-spacing: -1.5px; margin-bottom: 5px; }
    ...
    .section-title { font-size: 15px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; color: #000000; margin-top: 25px; margin-bottom: 15px; padding-bottom: 8px; border-bottom: 3px solid #000000; }
    .meta-tag { background-color: #f1f5f9; color: #000000; padding: 6px 14px; border-radius: 6px; font-weight: 700; font-size: 13px; border: 1px solid #cbd5e1; margin-right: 8px; display: inline-block; }
    .secretaria-header { font-size: 18px; font-weight: 800; color: #000000; margin-top: 20px; margin-bottom: 5px; padding-left: 8px; border-left: 6px solid #000000; }
    .metric-container { background-color: #ffffff; border: 2px solid #000000; border-radius: 12px; padding: 22px; margin-bottom: 15px; display: flex; flex-direction: column; justify-content: center; min-height: 120px; }
...
try:
    ...
    if not df.empty:
        ...
        # Montagem do menu lateral ultra enxuto
        fontes = sorted(list(set([str(f).strip() for f in df['fonte_clean'].unique() if str(f).strip() not in ['', 'nan']])))
        
        st.sidebar.markdown("<h3 style='margin-top:0; font-size:18px; color:#000000;'>Filtro Principal</h3>", unsafe_allow_html=True)
        fonte_sel = st.sidebar.selectbox("Selecione a Fonte Orçamentária:", options=fontes, index=0)
        
        # Criação de abas estruturais para organizar as consultas sem gerar gargalos de processamento
        tab_ativa, tab_geral, tab_deputados, tab_secretarias = st.tabs([
            f"🎯 Fonte Ativa: {fonte_sel}", 
            "🌐 Panorama Geral", 
            "🔍 Por Deputado", 
            "🏛️ Por Secretaria"
        ])
        
        # ABA 1: FOCO NA FONTE SELECIONADA (PROCESSAMENTO INSTANTÂNEO)
        with tab_ativa:
            if fonte_sel != "":
                ...
                if not df_final.empty:
                    ...
                    st.markdown(f'''<div class='kpi-row-container'>
                        <div class='kpi-card-head'>
                            ...
                            <div class='kpi-value'>{fmt(saldo_exclusivo_fonte)} <span style="font-size:12px; font-weight:600; color:#475569;">Disponível</span></div>
                        </div>
                        <div class='kpi-card-head-blue'>
                            <div class='kpi-label' style='color:#1e40af;'>🏦 Conta Corrente Vinculada: {conta_vinculada}</div>
                            <div class='kpi-value' style='color:#2563eb;'>{fmt(saldo_real_banco_total)} <span style="font-size:12px; font-weight:600; color:#475569;">Saldo no Banco</span></div>
                        </div>
                    ...
                    if len(secretarias) > 1:
                        ...
                        st.markdown(f'''<table class='extrato-table'>
                            ...
                            <tr class='extrato-row'><td>(-) DESPESAS CONTRATADAS TOTAIS</td><td class='extrato-cell-val' style='color:#dc2626;'>{fmt(global_despesa_bruta)}</td></tr>
                            <tr class='extrato-row-final' style='background-color:#ecf2ff;'><td class='extrato-cell-label'>(=) SALDO REAL ACUMULADO DISPONÍVEL</td><td class='extrato-cell-val' style='color:{"#059669" if global_saldo >= 0 else "#dc2626"};'>{fmt(global_saldo)}</td></tr>
                    ...
                    for sec in secretarias:
                        ...
                        st.markdown(f'''<table class='extrato-table'>
                            ...
                            <tr class='extrato-row'><td class='extrato-cell-label'>(+) RENDIMENTOS DA CONTA</td><td class='extrato-cell-val' style='color:#2563eb;'>{fmt(sec_rendimento)}</td></tr>
                            <tr class='extrato-row'><td class='extrato-cell-label'>(-) DESPESAS LIQUIDADAS (NF BRUTA)</td><td class='extrato-cell-val' style='color:#dc2626;'>{fmt(sec_despesa_bruta)}</td></tr>
                            <tr class='extrato-row-final'><td class='extrato-cell-label'>(=) SALDO ATUAL LIVRE</td><td class='extrato-cell-val' style='color:#059669;'>{fmt(sec_saldo)}</td></tr>
                        </table>''', unsafe_allow_html=True)
                    ...
                    if not df_exibicao.empty:
                        df_tab = pd.DataFrame({
                            'DATA': df_exibicao['DATA_LANCAMENTO'], 
                            'EMPENHO': df_exibicao['EMPENHO_COL'], 
                            'NOTA': df_exibicao['NOTA_COL'], 
                            'VALOR BRUTO NF': df_exibicao['Valor Bruto da NF'].apply(fmt), 
                            'Links/PDF': df_exibicao['PDF_GERAL']
                        })
                        st.dataframe(df_tab, use_container_width=True, hide_index=True)

        # ABA 2: PANORAMA GERAL MUNICIPAL
        with tab_geral:
            st.markdown("<div class='section-title' style='color:#1e3a8a; border-bottom:3px solid #1e3a8a;'>🌐 Panorama Consolidado Histórico das Fontes</div>", unsafe_allow_html=True)
            ...
            c_g2.markdown(f'''<div class='metric-container' style='border-color:#dc2626; border-left: 6px solid #dc2626;'><div>💸 Total Já Liquidado Municipal (Gasto Bruto)</div><div style="font-size:24px; font-weight:800; color:#dc2626;">{fmt(tot_global_gasto)}</div></div>''', unsafe_allow_html=True)
            ...
            fig_tempo = go.Figure(go.Scatter(x=df_cronologico['ano_mov'], y=df_cronologico['Saldo_Acumulado_Real'], mode='lines+markers+text', line=dict(color='#059669', width=4), text=[f"<b>{fmt(v)}</b>" for v in df_cronologico['Saldo_Acumulado_Real']], textposition="top center"))
            fig_tempo.update_layout(title="<b>Evolução Cronológica do Saldo Disponível Real em Caixa</b>", plot_bgcolor='#ffffff', paper_bgcolor='#ffffff', height=260, margin=dict(l=10,r=10,t=40,b=10), xaxis=dict(type='category'), yaxis=dict(showgrid=True, gridcolor='#e2e8f0'))
            st.plotly_chart(fig_tempo, use_container_width=True)

        # ABA 3: FILTRO AVANÇADO PARLAMETARES
        with tab_deputados:
            st.markdown("<div class='section-title'>📊 Recursos Vinculados por Parlamentar</div>", unsafe_allow_html=True)
            ...
            df_dep.columns = ['Parlamentar / Deputado', 'Fonte', '(+) Valor Repasse', '(-) Total Já Gasto', '(=) Saldo Disponível', 'Descrição do Objeto']
            st.dataframe(df_dep, use_container_width=True, hide_index=True)

        # ABA 4: PORTFÓLIO DE PASTAS E SECRETARIAS
        with tab_secretarias:
            st.markdown("<div class='section-title'>🏛️ Recursos Distribuidos por Portfólio de Secretarias</div>", unsafe_allow_html=True)
            ...
            df_sec.columns = ['Secretaria / Pasta', 'Fonte Orçamentária', '(+) Repasses Destinados', '(-) Valores Gastos', '(=) Saldo Real']
            st.dataframe(df_sec, use_container_width=True, hide_index=True)
            
except Exception as e:
    st.error(f"Erro de carregamento geral: {e}")
