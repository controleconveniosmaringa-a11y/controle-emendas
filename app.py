...
if st.session_state.pagina_atual == 'menu_principal':
    ...
    st.button("⬅️ Voltar", on_click=mudar_pagina, args=('credito',))
    ...
    st.button("⬅️ Voltar ao Menu", on_click=mudar_pagina, args=('menu_principal',))
    ...
    if not df_conv.empty:
        ...
        with tab_conv_fonte:
            ...
            if fontes_rec:
                f_final = st.selectbox("Escolha a Fonte:", options=fontes_rec)
        ...
        with tab_conv_analista:
            if 'RESPONSÁVEL' in df_conv_tela.columns:
                ...
                if analistas:
                    a_final = st.selectbox("Escolha o Analista:", options=analistas)
    ...
    if not df.empty:
        ...
        aba_selecionada = st.radio("Navegação:", options=["🎯 Por Fonte", "📋 Por Plano", "🏛️ Por Secretaria", "🔍 Por Deputado"], horizontal=True, label_visibility="collapsed")
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        if aba_selecionada == "🎯 Por Fonte":
            ...
            if fontes:
                ...
                if fonte_final:
                    ...
                    anos_f = ["Histórico Completo"] + sorted(list(set([str(a) for a in df_f['ano_mov'].unique() if a not in ['', 'nan']])))
                    ano_f_sel = st.selectbox("Exercício Fiscal:", options=anos_f)
                    ...
                    tot_ent = float(saldo_f['repasse'].sum() + saldo_f['rendimento'].sum())
                    tot_sai = float(saldo_f['bruto'].sum())
                    sal_f = tot_ent - tot_sai
                    
                    st.markdown(f'''<div class='kpi-row-container'><div class='kpi-card-head' style='border-left: 5px solid var(--success-val);'><div class='kpi-label'>🎯 Saldo Fonte</div><div class='kpi-value' style='color:var(--success-val);'>{fmt(sal_f)}</div></div></div>''', unsafe_allow_html=True)
                    ...
                    st.dataframe(fluxo_f, use_container_width=True)

        elif aba_selecionada == "📋 Por Plano":
            planos = sorted([str(p).upper() for p in df['plano_clean'].unique() if str(p).strip() not in ['', 'nan']])
            if planos:
                p_fin = st.selectbox("Selecione o Plano:", options=planos)
                if p_fin: st.dataframe(df[df['plano_clean'].str.upper() == p_fin], use_container_width=True)
            ...
            if secs:
                s_fin = st.selectbox("Selecione a Secretaria:", options=secs)
            ...
            if deps:
                d_fin = st.selectbox("Selecione o Deputado:", options=deps)
